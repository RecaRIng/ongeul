...
function makePurpose(analysis: BackendAnalysis): string {
  const title = analysis.document.title;
  if (/간담회/.test(title)) return '학부모 간담회 참여를 위한 안내문입니다';
  if (/모집/.test(title)) return '모집 내용을 확인하기 위한 안내문입니다';
  if (/신청|제출/.test(title)) return '신청 또는 제출이 필요한 내용을 알려주는 문서입니다';
  if (/과제|학습|발표/.test(title)) return '학습 활동을 준비하기 위한 안내문입니다';
  return analysis.actionSteps[0]?.action || '문서 내용을 확인하기 위한 안내문입니다';
}
...
export default function App() {
  ...
  const mapBackendResponse = (analysis: BackendAnalysis) => {
    const summaryLines = [
      ...
      analysis.coreFields.audience ? `대상: ${analysis.coreFields.audience}` : '',
      analysis.coreFields.contact ? `문의: ${analysis.coreFields.contact}` : '',
