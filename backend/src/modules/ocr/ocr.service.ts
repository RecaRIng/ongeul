import { callClovaOcr } from './clova.client';
import { normalizeClovaOcrResult } from './ocr.normalizer';
import type { Base64ImageInput, ImageFileLike, NormalizedOcrResult } from './ocr.types';
import { getImageFormat, validateImage } from './ocr.utils';
...
export async function extractTextFromBase64Image(input: Base64ImageInput): Promise<NormalizedOcrResult> {
  ...
  return extractTextFromImage({
    buffer,
    originalname,
    mimetype: `image/${input.imageFormat === 'jpg' ? 'jpeg' : input.imageFormat}`,
    size: buffer.length
  });
}
