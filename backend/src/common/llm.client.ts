...
function findLearningActions(text: string): string[] {
  ...
  if (/책을\s*골라/.test(text)) {
    addUniqueAction(actions, '읽은 책 고르기');
  }
  if (/독서록을\s*써\s*오세요/.test(text)) {
    addUniqueAction(actions, '독서록 쓰기');
  }
  if (/책\s*제목.*줄거리.*느낀\s*점/.test(text)) {
    addUniqueAction(actions, '책 제목, 줄거리, 느낀 점 적기');
  }
  if (/보고서를\s*작성하세요/.test(text)) {
    addUniqueAction(actions, '보고서 작성하기');
  }
  if (/관찰\s*날짜.*장소.*특징/.test(text)) {
    addUniqueAction(actions, '관찰 날짜, 장소, 특징 정리하기');
  }
  if (/식물의\s*특징.*정리해야\s*합니다/.test(text)) {
    addUniqueAction(actions, '식물의 특징 정리하기');
  }
  ...
  if (/공공기관을\s*조사하여/.test(text)) {
    addUniqueAction(actions, '공공기관 조사하기');
  }
  if (/발표\s*자료를\s*만들어\s*오세요/.test(text)) {
    addUniqueAction(actions, '발표 자료 만들기');
  }
  if (/\d+분\s*발표를\s*준비합니다/.test(text)) {
    const match = text.match(/(\d+분)\s*발표를\s*준비합니다/);
    addUniqueAction(actions, `${match?.[1] ?? ''} 발표 준비하기`.trim());
  }
  if (/문제를\s*풀어오세요/.test(text)) {
    addUniqueAction(actions, '문제 풀어오기');
  }
  if (/답을\s*쓰세요/.test(text)) {
    addUniqueAction(actions, '답 쓰기');
  }
...
export function safeJsonParse<T = unknown>(response: string): T | null {
  // 입력 검증: 문자열이 아니거나 빈 문자열이면 null 반환
  ...
  // 두 번째 시도: JSON 객체 또는 JSON 배열 부분만 추출하여 파싱
  try {
    ...
    // 끝 위치가 없거나 시작 위치보다 뒤에 있지 않으면 유효하지 않음
